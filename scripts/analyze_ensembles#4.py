...
def main(name):
    ...
    out = {}
    try:
        print('aaaaaaaaaaaaaaaaaa',name)
        ref_aa = mdtraj.load(f'{args.atlas_dir}/{name}/{name}.pdb')
        topfile = f'{args.atlas_dir}/{name}/{name}.pdb'
        # ref_aa = mdtraj.load(f'{args.atlas_dir}/{name}.pdb')
        # topfile = f'{args.atlas_dir}/{name}.pdb'
        ...
        traj_aa = mdtraj.load(f'{args.atlas_dir}/{name}/{name}.xtc', top=topfile)
        print(f'Loaded {traj_aa.n_frames} reference frames')
        
        print('Loading AF2 conformers')
        #aftraj_aa = mdtraj.load(f'{args.pdbdir}/{name}.pdb')
        aftraj_aa = mdtraj.load(f'{args.pdbdir}/{name}.xtc', top=f'{args.pdbdir}/{name}.pdb')
        ...
        n_use = min(100, traj_aa.n_frames, aftraj_aa.n_frames)
        traj_aa = traj_aa[:n_use]
        aftraj_aa = aftraj_aa[:n_use]
        print(f'Using first {n_use} frames for both traj_aa and aftraj_aa')
        ...
        RAND1 = np.random.randint(0, traj_aa.n_frames, 100)
        RAND2 = np.random.randint(0, traj_aa.n_frames, 100)
        RAND1K = np.arange(traj_aa.n_frames)   # 0..99
...
single_id = pdb_id[0] 
with open(f"{args.pkl}/{single_id}.pkl", "wb") as f:
    pickle.dump(out, f)