...
def atom14_to_atom37(atom14: np.ndarray, aatype, atom14_mask=None):
    # (B, N, 37)
    indices = rc.RESTYPE_ATOM37_TO_ATOM14[aatype]

    # expand to gather xyz coords
    indices_expanded = indices[..., None]  # (B, N, 37, 1)

    # gather
    atom37 = np.take_along_axis(
        atom14,
        indices_expanded,
        axis=-2
    )  # → (B, N, 37, 3)
    ...
    if atom14_mask is not None:
        atom37_mask = np.take_along_axis(
            atom14_mask,
            indices,
            axis=-1
        )
